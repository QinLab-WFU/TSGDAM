...
class Fire_Down(nn.Module):
    def __init__(self, kernel_size, in_channels, inner_channels, out_channels):
        ...
        dilations = [1, 3, 5]
        self.Conv1 = nn.Conv2d(in_channels, inner_channels, kernel_size=kernel_size, stride=1, padding=dilations[
            0], dilation=dilations[0])
        self.Conv4 = nn.Conv2d(in_channels, inner_channels, kernel_size=kernel_size, stride=1, padding=dilations[
            1], dilation=dilations[1])
        self.Conv8 = nn.Conv2d(in_channels, inner_channels, kernel_size=kernel_size, stride=1, padding=dilations[
            2], dilation=dilations[2])
...
class uNet(nn.Module):
    def __init__(self, n_classes, n_channels):
        ...
        self.Conv0 = self._transition(n_channels, 8)  # 1918
        ...
        self.tran0 = self._transition(96, 192)
        self.db0 = self._dense_block(192, 32)

        # self.up1 = self._up_block(256, 96, 96)  # 120
        self.up1 = self._up_block(192, 96, 96)  # 120
        ...
        self.conv5 = nn.Conv2d(16, n_classes, 3, stride=1, padding=1)
    ...
    def forward(self, x):
        ...
        down4 = self.tran0(down4)
        down4 = self.db0(down4)

        db_ = self.db1(self.up1(down4))
        up2 = self.act(self.bn1(self.conv1(torch.cat([db_, down3], dim=1))))
        del down4, down3

        # db_ = self.db1(self.up1(down5))
        # up1 = self.act(self.bn1(self.conv1(torch.cat([db_, down4], dim=1))))
        # del down5, down4
