...
class BaseOptions():
    ...
    def initialize(self, parser):
        ...
        parser.add_argument('--dataset_mode', type=str, default='aligned',
                            help='chooses how datasets are loaded. [aligned | aligned_resized | single]')
        ...
        parser.add_argument('--skip', type=int, default=0,
                            help='Define whether the guidance layer is skipped. Useful when using multiGPUs.')
        parser.add_argument('--fuse', type=int, default=0,
                            help='Fuse may encourage large patches shifting when using \'patch_soft_shift\'')
        parser.add_argument('--gan_type', type=str, default='vanilla', help='wgan_gp, '
                                                                            'lsgan, '
                                                                            'vanilla, '
                                                                            're_s_gan (Relativistic Standard GAN), '
                                                                            're_avg_gan')
        parser.add_argument('--gan_weight', type=float, default=0.02, help='the weight of gan loss')
        parser.add_argument('--style_weight', type=float, default=120, help='the weight of style loss')
        parser.add_argument('--content_weight', type=float, default=1, help='the weight of content loss')
        parser.add_argument('--discounting', type=int, default=1,
                            help='the loss type of mask part, whether using discounting l1 loss or normal l1')
        parser.add_argument('--use_spectral_norm_D', type=int, default=1,
                            help='whether to add spectral norm to D, it helps improve results')
        parser.add_argument('--use_spectral_norm_G', type=int, default=0,
                            help='whether to add spectral norm in G. Seems very bad w hen adding SN to G')
        parser.add_argument('--expansion_factor', type=float, default=2.66, help='Expansion factor for FeedForward')
